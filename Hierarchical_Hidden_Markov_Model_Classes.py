...
# # Assuming you have already defined and run your HHMM model
# # and have the following variables available:
# # historical_data, O, model_posterior, maxT, S0

# # Create the result dataframe
# result_df = historical_data.reset_index()
# if 'index' in result_df.columns:
#     result_df.rename(columns={'index': 'date'}, inplace=True)
# elif result_df.index.name == 'date':
...
#     # For date column, drop rows with NaN dates as they are crucial
#     if date_column:
#         df = df.dropna(subset=[date_column])
    
#     # For OHLCV data, forward fill, then backward fill
#     for col in ['Open', 'High', 'Low', 'Close', 'Volume']:
#         if col in df.columns:
#             df[col] = df[col].fillna(method='ffill').fillna(method='bfill')
    
#     # For returns, calculate based on close prices if NaN
#     if 'returns' in df.columns and 'Close' in df.columns:
#         df['returns'] = df['returns'].fillna(np.log(df['Close'] / df['Close'].shift(1)))
    
#     # For states, fill NaN with the most common state
#     if 'states' in df.columns:
#         df['states'] = df['states'].fillna(df['states'].mode()[0])
    
#     # For posterior probabilities, distribute evenly if NaN
...
# print("NaN count in each column after handling:")
# print(nan_counts)

# # Save historical data with posterior probabilities
# result_df.to_csv('historical_data_with_posterior.csv', index=False, date_format='%Y-%m-%d %H:%M:%S')
# print("CSV file 'historical_data_with_posterior.csv' has been saved.")
...
#     'date': pd.date_range(start=historical_data.index[-1] + pd.Timedelta(hours=1), periods=maxT, freq='H'),
#     'min': lower,
#     'max': upper,
#     'mid': mid
# })
# forecast_df.to_csv('forecast_data.csv', index=False, date_format='%Y-%m-%d %H:%M:%S')
# print("CSV file 'forecast_data.csv' has been saved.")

# # Data validation
# required_columns = ['Open', 'High', 'Low', 'Close', 'Volume', 'returns', 'states'] + [f'posterior_state_{i}' for i in range(model_posterior.shape[0])]
# missing_columns = set(required_columns) - set(result_df.columns)
# if missing_columns:
#     print(f"Warning: Missing required columns: {missing_columns}")

# # Final check for NaN values
# remaining_nan = result_df.columns[result_df.isna().any()].tolist()
# if remaining_nan:
#     print(f"\nWarning: NaN values still present in columns: {remaining_nan}")
#     print("Consider additional handling or investigation for these columns.")
# else:
...
# def plot_hmm_classification_plotly(historical_data, posterior, symbol):
#     # Determine the most likely state at each time point
...
#     # Create subplots: one for price, one for state probabilities
#     fig = make_subplots(rows=2, cols=1, shared_xaxes=True, 
#                         vertical_spacing=0.1, 
#                         subplot_titles=("Price with HMM State Classification", "State Probabilities"))
    
#     # Plot the price data, color-coded by state
#     for state in range(num_states):
#         mask = most_likely_states == state
#         fig.add_trace(
#             go.Scatter(
#                 x=historical_data.index[mask],
#                 y=historical_data[symbol][mask],
#                 mode='markers',
...
#     # Add a line plot for the continuous price data
#     fig.add_trace(
#         go.Scatter(
#             x=historical_data.index,
#             y=historical_data[symbol],
#             mode='lines',
...
#                 name=f'State {state} Probability'
#             ),
#             row=2, col=1
#         )
...
#     fig.update_layout(
#         height=800,
#         title_text=f"HMM State Classification for {symbol}",
#         xaxis2_title="Date",
#         yaxis_title="Price",
...
#     fig.write_html("hmm_classification_plot.html")

# # Usage example:
# plot_hmm_classification_plotly(historical_data, model_posterior, symbol)#%%
